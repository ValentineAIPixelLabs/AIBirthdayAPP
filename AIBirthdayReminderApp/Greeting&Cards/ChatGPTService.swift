...
""")
    ...
    /// Генерирует креативный промт для открытки на основе данных контакта (использует GPT-4о)
    ///
    /// Формирует визуальный промт для генерации поздравительной открытки на основе информации о контакте.
    /// Промт описывает насыщенную мини-сцену для AI-генерации изображения, не используя личные данные буквально,
    /// а опираясь на ассоциации, сценарии, аллюзии, стиль и предпочтения, выявленные по данным контакта.
    /// - Parameters:
    ///   - contact: Контакт, на основе которого составляется промт
    ///   - apiKey: Ключ OpenAI
    ///   - completion: Коллбек с результатом (строка промта или ошибка)
    func generateCreativePrompt(for contact: Contact, apiKey: String, completion: @escaping (Result<String, Error>) -> Void) {
        // Формируем поля данных контакта
        var infoLines = [String]()
        infoLines.append("Name: \(contact.name)")
        if let nickname = contact.nickname, !nickname.isEmpty {
            infoLines.append("Nickname: \(nickname)")
        }
        if let gender = contact.gender, !gender.isEmpty {
            infoLines.append("Gender: \(gender)")
        }
        let calendar = Calendar.current
        let currentYear = calendar.component(.year, from: Date())
        if let year = contact.birthday?.year {
            let age = currentYear - year
            infoLines.append("Age: \(age)")
        }
        if let relation = contact.relationType, !relation.isEmpty {
            infoLines.append("Relation type: \(relation)")
        }
        if let occupation = contact.occupation, !occupation.isEmpty {
            infoLines.append("Occupation: \(occupation)")
        }
        if let hobbies = contact.hobbies, !hobbies.isEmpty {
            infoLines.append("Hobbies: \(hobbies)")
        }
        if let leisure = contact.leisure, !leisure.isEmpty {
            infoLines.append("Leisure: \(leisure)")
        }
        if let info = contact.additionalInfo, !info.isEmpty {
            infoLines.append("Additional Info: \(info)")
        }
        let infoBlock = infoLines.joined(separator: "\n")
        
        let systemPrompt = """
        Ты — эксперт по созданию кинематографичных AI-промтов для генерации поздравительных открыток. Используй информацию о человеке только как источник вдохновения: не вставляй личные данные напрямую, а строй ассоциации, сценарии, аллюзии.
        
        Перед тобой описание человека по полям:
        — Имя, прозвище: используй для поиска известных личностей/персонажей, создавай ассоциации или креативные связи.
        — Отношения: определяй характер и стиль сцены (шутливо, серьёзно, дружески, формально и т.д.).
        — Пол: выбирай детали, которые могут понравиться человеку данного пола.
        — Возраст: если человек молодой — допускаются детские или молодёжные темы, если взрослый — более зрелые или сложные сюжеты.
        — Род деятельности, увлечения, как проводит свободное время, дополнительная информация: это ключ к предпочтениям и характерам, основа для выбора тематики и атмосферы открытки.
        
        Твой промт должен быть коротким (до 120 слов), описывать насыщенную мини-сцену для AI, включать:
        [герой/объект/известный персонаж], [действие/ситуация], [локация], [яркие детали], [визуальный стиль], [эффекты/свет/ракурс], [эмоции/настроение], [техника/камера].
        
        Используй современные паттерны: кроссоверы, иронию, кино-язык, мемы, аллюзии, midjourney/dall-e style, фотореализм, описания света, техники, атмосферы. Не поздравляй, не используй имя и данные буквально, не объясняй выбор — только визуальная сцена.
        """
